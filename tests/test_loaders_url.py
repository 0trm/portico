...
<article>
  ...
  <p>And a third paragraph with another distinct claim worth quoting in a portico.</p>
</article>
</body></html>
"""
...
def test_load_url_happy_path() -> None:
    with patch("arqii.loaders.url.httpx.get", return_value=_Resp(200, SAMPLE_HTML)):
...
def test_load_url_4xx_raises_f1_remote() -> None:
    with (
        patch("arqii.loaders.url.httpx.get", return_value=_Resp(404, "")),
...
def test_load_url_5xx_raises_f1_remote() -> None:
    with (
        patch("arqii.loaders.url.httpx.get", return_value=_Resp(503, "")),
        pytest.raises(F1RemoteInaccessible),
    ):
...
def test_load_url_connect_error_raises_f1_network() -> None:
    ...
    with (
        patch("arqii.loaders.url.httpx.get", side_effect=boom),
...
def test_load_url_unextractable_raises_f2() -> None:
    with (
        patch("arqii.loaders.url.httpx.get", return_value=_Resp(200, "<html></html>")),
